...
def get_clients(db: Session = Depends(deps.get_db)):
    ...
    if not db_client:
        raise HTTPException(status_code=204, detail="No existen clientes.")
    return db_client
...
def get_clients(cellphone: str, db: Session = Depends(deps.get_db)):
    ...
    if not db_client:
        raise HTTPException(status_code=204, detail="El cliente no existe.")
    return db_client
...
def create_client(client: ClientCreate, db: Session = Depends(deps.get_db)):
    db_client = clients.create_client(db, client.telefono, client=client)
    if not db_client:
